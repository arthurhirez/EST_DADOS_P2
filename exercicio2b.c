...
int insert_hash_rehash(hashT *table, string element, unsigned size, unsigned *n_colision){
...
int search_hash_rehash(hashT *table, string element, unsigned size, unsigned *n_found){
    unsigned key = converter(element);
    for(unsigned i = 0; i < size; i++){
        unsigned key_hash = rehash(key, i, size);
        // printf("[%d]:\t%20s\t=\t%d -> %d\n", i, element, key, key_hash);
